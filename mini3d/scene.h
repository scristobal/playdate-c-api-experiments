...
struct ShapeInstance {
  ...
  int inverted : 1; // transformation flipped it across a plane, so we need to reverse backface
                    // check
#if ENABLE_Z_BUFFER
  int useZBuffer : 1;
#endif
...
typedef struct {
  ...
  // location of the Z vanishing point on the screen. (0,0) is top left corner, (1,1) is bottom
  // right. Defaults to (0.5,0.5)
  float centerx;
  float centery;
