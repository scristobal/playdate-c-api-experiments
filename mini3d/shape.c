...
void Shape3D_addFace(Shape3D *shape, Point3D *a, Point3D *b, Point3D *c, Point3D *d,
                     float colorBias) {
...
void Shape3D_setClosed(Shape3D *shape, int flag) { shape->isClosed = flag; }

#if ENABLE_ORDERING_TABLE
void Shape3D_setOrderTableSize(Shape3D *shape, int size) { shape->orderTableSize = size; }
#endif
