...
#if ENABLE_Z_BUFFER
static void drawFragment_zbuf(uint32_t *row, uint16_t *zbrow, int x, int endx, uint32_t z,
                              int32_t dzdx, uint32_t color) {
...
#if ENABLE_Z_BUFFER
LCDRowRange drawLine_zbuf(uint8_t *bitmap, int rowstride, Point3D *p1, Point3D *p2, int thick,
                          uint8_t pattern[8]) {
  if (p1->y > p2->y) {
    Point3D *tmp = p1;
    p1 = p2;
  ...
  while (y <= endy) {
    ...
    if (dx < 0) {
      z += dzdy;
      drawFragment_zbuf((uint32_t *)&bitmap[y * rowstride], &zbuf[y * LCD_COLUMNS], x1 >> 16,
                        (x >> 16) + thick, z, dzdx, color);
    } else {
      drawFragment_zbuf((uint32_t *)&bitmap[y * rowstride], &zbuf[y * LCD_COLUMNS], x >> 16,
                        (x1 >> 16) + thick, z, dzdx, color);
      z += dzdy;
    }
...
LCDRowRange drawLine(uint8_t *bitmap, int rowstride, Point3D *p1, Point3D *p2, int thick,
                     uint8_t pattern[8]) {
  if (p1->y > p2->y) {
    Point3D *tmp = p1;
    p1 = p2;
...
static void fillRange(uint8_t *bitmap, int rowstride, int y, int endy, int32_t *x1p, int32_t dx1,
                      int32_t *x2p, int32_t dx2, uint8_t pattern[8]) {
...
#if ENABLE_Z_BUFFER
static void fillRange_zbuf(uint8_t *bitmap, int rowstride, int y, int endy, int32_t *x1p,
                           int32_t dx1, int32_t *x2p, int32_t dx2, uint32_t *zp, int32_t dzdy,
                           int32_t dzdx, uint8_t pattern[8]) {
  int32_t x1 = *x1p, x2 = *x2p;
  uint32_t z = *zp;
...
LCDRowRange fillTriangle(uint8_t *bitmap, int rowstride, Point3D *p1, Point3D *p2, Point3D *p3,
                         uint8_t pattern[8]) {
  // sort by y coord
...
#if ENABLE_Z_BUFFER
LCDRowRange fillTriangle_zbuf(uint8_t *bitmap, int rowstride, Point3D *p1, Point3D *p2, Point3D *p3,
                              uint8_t pattern[8]) {
  sortTri(&p1, &p2, &p3);
  ...
  fillRange_zbuf(bitmap, rowstride, p1->y, MIN(LCD_ROWS, p2->y), &x1, dx1, &x2, dx2, &z, dzdy, dzdx,
                 pattern);
...
LCDRowRange fillQuad(uint8_t *bitmap, int rowstride, Point3D *p1, Point3D *p2, Point3D *p3,
                     Point3D *p4, uint8_t pattern[8]) {
...
#if ENABLE_Z_BUFFER
LCDRowRange fillQuad_zbuf(uint8_t *bitmap, int rowstride, Point3D *p1, Point3D *p2, Point3D *p3,
                          Point3D *p4, uint8_t pattern[8]) {
  // XXX - implement with 3 fillRange_zbuf() calls
