...
Matrix3D identityMatrix = {.isIdentity = 1,
                           .inverting = 0,
                           .m = {{1, 0, 0}, {0, 1, 0}, {0, 0, 1}},
                           .dx = 0,
                           .dy = 0,
...
Matrix3D Matrix3DMake(float m11, float m12, float m13, float m21, float m22, float m23, float m31,
                      float m32, float m33, int inverting) {
  return (Matrix3D){.isIdentity = 0,
                    .inverting = inverting,
...
float Matrix3D_getDeterminant(Matrix3D *m) {
  return m->m[0][0] * m->m[1][1] * m->m[2][2] + m->m[0][1] * m->m[1][2] * m->m[2][0] +
         m->m[0][2] * m->m[1][0] * m->m[2][1] - m->m[2][0] * m->m[1][1] * m->m[0][2] -
