...
void Scene3DNode_deinit(Scene3DNode *node) {
...
void Scene3DNode_addShapeWithOffset(Scene3DNode *node, Shape3D *shape, Vector3D offset) {
  Scene3DNode_addShapeWithTransform(node, shape,
                                    Matrix3DMakeTranslate(offset.dx, offset.dy, offset.dz));
}
...
static void Scene3D_updateShapeInstance(Scene3D *scene, ShapeInstance *shape, Matrix3D xform,
                                        float colorBias, RenderStyle style) {
...
void Scene3D_updateNode(Scene3D *scene, Scene3DNode *node, Matrix3D xform, float colorBias,
                        RenderStyle style, int update) {
  if (!node->isVisible)
    return;
...
void Scene3D_setGlobalLight(Scene3D *scene, Vector3D light) { scene->light = light; }
...
Scene3DNode *Scene3D_getRootNode(Scene3D *scene) { return &scene->root; }

static int getShapesAtNode(Scene3D *scene, Scene3DNode *node, int count) {
  if (!node->isVisible)
    return count;
...
#define SHAPELIST_INCREMENT 16
      scene->shapelist = m3d_realloc(
          scene->shapelist, (scene->shapelistsize + SHAPELIST_INCREMENT) * sizeof(ShapeInstance *));
...
static Pattern patterns[] = {{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
...
static inline void drawShapeFace(Scene3D *scene, ShapeInstance *shape, uint8_t *bitmap,
                                 int rowstride, FaceInstance *face) {
...
static inline void drawFilledShape(Scene3D *scene, ShapeInstance *shape, uint8_t *bitmap,
                                   int rowstride) {
#if ENABLE_ORDERING_TABLE
...
static inline void drawWireframe(Scene3D *scene, ShapeInstance *shape, uint8_t *bitmap,
                                 int rowstride) {
  int f;
  ...
  for (f = 0; f < shape->nFaces; ++f) {
    ...
    if ((x1 < 0 && x2 < 0 && x3 < 0 && x4 < 0) ||
        (x1 >= WIDTH && x2 >= WIDTH && x3 >= WIDTH && x4 >= WIDTH) ||
        (y1 < 0 && y2 < 0 && y3 < 0 && x4 < 0) ||
...
void Scene3D_draw(Scene3D *scene, uint8_t *bitmap, int rowstride) {
#if ENABLE_Z_BUFFER
  scene->zmin = 1e23f;
#endif
